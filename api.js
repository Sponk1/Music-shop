...
// Получение фото по ID музыкального инструмента
app.get("/photos/:music_instrument_id", async (req, res) => {
  try {
    const { music_instrument_id } = req.params;
    const result = await pool.query(
      "SELECT file_url, description FROM photos WHERE music_instrument_id = $1",
      [music_instrument_id]
    );
    if (result.rows.length > 0) {
      res.json(result.rows);
...
app.post("/photos", async (req, res) => {
  try {
    const { music_instrument_id, file_url, description } = req.body;
    const newPhoto = await pool.query(
      "INSERT INTO photos (music_instrument_id, file_url, description) VALUES ($1, $2, $3) RETURNING *",
      [music_instrument_id, file_url, description]
    );
...
app.delete("/photos/:photo_id", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM photos WHERE id = $1", [photo_id]);
